...
import "./styles/basictabelecommerce.css";

const BasicTableEcommerce = ({
  columns,
  data,
  fetchData,
  loading,
  ...
  return (
    <>
      <Table
        className="react-basic-table-ecommerce"
        responsive
        {...getTableProps()}
      >
        <thead>
          {headerGroups.map((headerGroup) => (
            <tr {...headerGroup.getHeaderGroupProps()}>
              {headerGroup.headers.map((column) => (
                <th
                  className="th-react-table"
                  {...column.getHeaderProps(column.getSortByToggleProps())}
                >
                  {column.render("Header")}
                  <span
                    style={{
                      marginLeft: column.marginleft ?? "10px",
                      position: "relative",
                    }}
                  >
                    {column.isSorted ? (
...
export default BasicTableEcommerce;
